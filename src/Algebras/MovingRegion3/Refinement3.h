...
This file contains classes and functions for use within set operators 
(e.g. intersection, inside,... )


*/

/*
1.0 Precise Time Interval used in RefinementPartition2
...
public:
      ...
      rc(t.rc)
  {
    ...
    start = start + mpq_class(t.start.ToDouble());
    start.canonicalize();
    
    end = p.GetPreciseFinalInstant(preciseInstants);
    end = end + mpq_class(t.end.ToDouble());
    end.canonicalize();
  }
...
/*
2.0 Precise UPoint holds the precise values calculated within the set operations
...
/*
3.0 Precise UBool holds the precise values calculated within the set operations
...
/*
4.0 Refinement Partition 2 for set operations with MPoint and MRegion2
