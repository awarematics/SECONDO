...
JPEGPicture::JPEGPicture(unsigned char *JPEGBuffer, 
                         unsigned long JPEGBufferSize)
{
      InitMembers();

      ReadJPEGHeader(JPEGBuffer, JPEGBufferSize, &m_ulWidth, 
                      &m_ulHeight, &m_bGrayScale);
...
void JPEGPicture::CreateRGBBuffer(unsigned char *JPEGBuffer, 
                                  unsigned long JPEGBufferSize)
{
      ...
      buffer_pos                  = 0;
      m_ulImageBufferSize      = cinfo.output_width * cinfo.output_height * 
...
void JPEGPicture::ReadJPEGHeader(unsigned char *JPEGBuffer, 
                                 unsigned long JPEGBufferSize, 
                                 unsigned long *Width, 
...
JPEGPicture* JPEGPicture::scale(unsigned long w, unsigned long h)
{
      ...
      for ( y=0; y<new_y; y++ ) {
        for ( x=0; x<new_x; x++ ) {
          ...
          src_pos = (unsigned long)(fx*(double)x)+ 
                       ((int)(fy*(double)y))*m_ulWidth;
          pic->m_pucImageBuffer[dest_pos * RGB_BYTES] = 
                          m_pucImageBuffer[src_pos * RGB_BYTES];
          pic->m_pucImageBuffer[(dest_pos * RGB_BYTES) + 1] = 
                          m_pucImageBuffer[(src_pos * RGB_BYTES) + 1];
          pic->m_pucImageBuffer[(dest_pos * RGB_BYTES) + 2] = 
                          m_pucImageBuffer[(src_pos * RGB_BYTES) + 2];
        }
      }
...
// by parameters
JPEGPicture* JPEGPicture::cut(unsigned long x, 
                              unsigned long y, 
                              unsigned long w, 
                              unsigned long h )
{
        ...
        for (x = 0; x < (int)m_ulWidth; ++x) {
          switch (mode) {
            ...
            case 0 :
             ...
             // 270 ° left
             case 3 :
               dest_pos = (((m_ulHeight - 1) - y) + 
                          (x * m_ulHeight)) * RGB_BYTES;
               break;
             }
...
JPEGPicture* JPEGPicture::mirror(bool dir)
{
      ...
      for (y = 0; y < (int)m_ulHeight; ++y)
      {
            ...
            {
                  // horizontal
                  dest_pos = ((m_ulHeight - y) * m_ulWidth * RGB_BYTES) - 
                              (m_ulWidth * RGB_BYTES);
            }
            ...
            for (x = 0; x < (int)m_ulWidth; ++x)
            {
                  ...
                  {
                        // vertical
                        dest_pos = ((m_ulWidth * y) + 
                                    (m_ulWidth - x - 1)) * RGB_BYTES;
                  }

                  pic->m_pucImageBuffer[dest_pos] = m_pucImageBuffer[src_pos];
                  pic->m_pucImageBuffer[dest_pos+1] = 
                                          m_pucImageBuffer[src_pos+1];
                  pic->m_pucImageBuffer[dest_pos+2] = 
                                          m_pucImageBuffer[src_pos+2];
                  src_pos            += RGB_BYTES;
                  dest_pos      += RGB_BYTES;
            }
      }
...
unsigned long JPEGPicture::GetWidth(unsigned char* JPEGBuffer, 
                                    unsigned long JPEGBufferSize)
{
      unsigned long width;
...
unsigned long JPEGPicture::GetHeight(unsigned char* JPEGBuffer, 
                                     unsigned long JPEGBufferSize)
{
      unsigned long height;
...
bool JPEGPicture::IsGrayScale(unsigned char* JPEGBuffer, 
                              unsigned long JPEGBufferSize)
{
      bool grayscale;
