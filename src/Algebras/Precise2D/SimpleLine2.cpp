...
SimpleLine2::SimpleLine2(const bool def, bool ldp, int xl, int yl, int xr,
  int yr, mpq_class pxl, mpq_class pyl, mpq_class pxr, mpq_class pyr) :
  StandardSpatialAttribute<2>(def), preciseCoordinates(0), segmentData(0),
  lrsArray(0), ordered(true), startSmaller(true), isCycle(false), bbox(false),
  currentSD(-1) {
...
SimpleLine2::SimpleLine2(const SimpleLine2& l) :
  StandardSpatialAttribute<2>(l.IsDefined()), preciseCoordinates(
    l.preciseCoordinates.getSize()), segmentData(l.segmentData.Size()),
    lrsArray(l.Size() / 2), ordered(true), startSmaller(true), isCycle(false),
    bbox(false), currentSD(-1) {
 Equalize(l);
...
void SimpleLine2::addSegment(bool ldp, int leftX, int leftY, int rightX,
  int rightY, mpq_class pLeftX, mpq_class pLeftY, mpq_class pRightX,
...
void SimpleLine2::addSegment(bool ldp, Point2* lp, Point2* rp, int edgeno) {
...
bool SimpleLine2::computePolyline() {
 ...
 while (pos < size) {
  ...
  while (pos < size
    && ((p1DX == p2DX) && p1DY == p2DY && (cmp(p1PX, p2PX) == 0)
      && (cmp(p1PY, p2PY) == 0))) {
   count++;
 ...
 while (noUnused > 0) {
  ...
  if (noUnused > 0) {
   ...
   if (partnerpos > 0 && !used[partnerpos - 1]) { // check left side
    ...
    if (p1DX == p2DX && p1DY == p2DY && (cmp(p1PX, p2PX) == 0)
      && (cmp(p1PY, p2PY) == 0)) { // extension found
     found = true;
     sdPos = partnerpos - 1;
    }
   }
...
int SimpleLine2::CompareSegment2(const SegmentData& seg1,
  const SegmentData& seg2, const Flob* preciseCoordOfSeg2) const {
...
ListExpr SimpleLine2::SimpleLine2Property() {
 ...
 nl->AppendText(ListRepr, "((<segment>*) <bool> ), where <segment> is "
   "((<xl> <xr> ( <pxl> <pyl>))(<xr> <yr> "
 ...
 return nl->TwoElemList(
   ...
   nl->FourElemList(nl->StringAtom("-> DATA"),
     nl->StringAtom(SimpleLine2::BasicType()), ListRepr, ExampleList));
}
