/*
 ...
 be activated and deactivated in config.h
...
*/
void setupParams(std::set<Face*> *faces, const char *prefix, int depth) {
    char offstr[10];
    char scalestr[12];
...
*/
vector<pair<Face *, Face *> > _matchFacesLua(std::set<Face*> *src,
                              std::set<Face*> *dst, int depth, string args) {
...
/*
  3.1 ~distance~ returns the distance of the center points of the
  bounding-boxes of two given regions.
...
LUA_FUNCTION(overlap) {
    ...
    Poly r1 = src->MakePoly(srctransform.first.x, srctransform.first.y,
...
/*
 3.6 ~points~ returns the list of cornerpoints of this face in a table.
...
/*
 3.7 ~centroid~ returns the centroid of the given face (holes are not taken
 into account).
