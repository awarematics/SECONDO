...
import viewer.hoese.algebras.network.Network;
 ...
 *
 */
public class Dsplnetwork extends DisplayGraph 
{
  /**
  ...
  {
		if(in_iIndex < m_xNetwork.getRouteCount())
    {
			return m_xNetwork.getRouteAt(in_iIndex).getRenderObject();
		} 
    else 
    {
			int iAdjustedIndex = in_iIndex - m_xNetwork.getRouteCount();

      Point2D.Double xPoint = m_xNetwork.getJunctionAt(iAdjustedIndex).getRenderObject();
  ...
  {
		return m_xNetwork.getRouteCount() + m_xNetwork.getJunctionCount();
	}
  ...
  {
    boolean bPointType = in_iIndex >= m_xNetwork.getRouteCount();
    return bPointType;
	}
  ...
  {
	  boolean bLineType = in_iIndex < m_xNetwork.getRouteCount();
    return bLineType;
	}
 ...
 public Rectangle2D.Double getBounds () 
 {
   ...
   for (int i = 0; i < m_xNetwork.getRouteCount(); i++) {
     Shape xShape = m_xNetwork.getRouteAt(i).getRenderObject();
   ...
   for (int i = 0; i < m_xNetwork.getJunctionCount(); i++) {
     Point2D.Double xShapeBounds = m_xNetwork.getJunctionAt(i).getRenderObject();
     if(xBounds == null){
       xBounds = new Rectangle2D.Double(xShapeBounds.getX(),
                                        xShapeBounds.getY(),
                                        0,
