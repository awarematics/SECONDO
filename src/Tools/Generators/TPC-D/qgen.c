/*
 * Sccsid:     @(#)qgen.c	9.1.1.35     5/1/96  11:45:58
...
usage(void)
{
printf("TPC-D Parameter Substitution (v. %d.%d.%d%s)\n", 
          VERSION,RELEASE,
            MODIFICATION,PATCH);
...
printf("\t-T\t\t-- use TIME table values for date substitutions\n");
...
process_options(int cnt, char **args)
{
    ...
    while((flag = getopt(cnt, args, "acdhi:n:Nl:o:p:r:s:t:Tvx")) != -1)
        switch(flag)
            {
...
main(int ac, char **av)
{
    ...
    if (flags & VERBOSE)
        fprintf(ofp, 
	    "-- TPC-D Parameter Substitution (Version %d.%d.%d%s)\n",
