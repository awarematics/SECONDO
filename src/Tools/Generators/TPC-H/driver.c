...
tdef tdefs[] =
{
	{"part_tbl", "part table", 200000, hd_part,
		{pr_part, ld_part}, sd_part, vrf_part, PSUPP, 0},
	{"partsupp_tbl", "partsupplier table", 200000, hd_psupp,
		{pr_psupp, ld_psupp}, sd_psupp, vrf_psupp, NONE, 0},
	{"supplier_tbl", "suppliers table", 10000, hd_supp,
		{pr_supp, ld_supp}, sd_supp, vrf_supp, NONE, 0},
	{"customer_tbl", "customers table", 150000, hd_cust,
		{pr_cust, ld_cust}, sd_cust, vrf_cust, NONE, 0},
	{"orders_tbl", "order table", 150000, hd_order,
		{pr_order, ld_order}, sd_order, vrf_order, LINE, 0},
	{"lineitem_tbl", "lineitem table", 150000, hd_line,
		{pr_line, ld_line}, sd_line, vrf_line, NONE, 0},
	{"orders_tbl", "orders/lineitem tables", 150000, hd_order_line,
		{pr_order_line, ld_order_line}, sd_order, vrf_order_line, LINE, 0},
	{"part_tbl", "part/partsupplier tables", 200000, hd_part_psupp,
		{pr_part_psupp, ld_part_psupp}, sd_part, vrf_part_psupp, PSUPP, 0},
	{"nation_tbl", "nation table", NATIONS_MAX, hd_nation,
		{pr_nation, ld_nation}, NO_LFUNC, vrf_nation, NONE, 0},
	{"region_tbl", "region table", NATIONS_MAX, hd_region,
...
/* M. Spiekermann. A global variable is introduced to signal that the
   last data record is written. This was done to limit code changes.
*/
int GLOB_LAST_RECORD = 0;
		  ...
		  case 'O':				/* optional actions */
			  ...
			  case 'h':				/* something unexpected */
				  fprintf (stderr,
					  ...
					  MODIFICATION, PATCH, SECONDO_MOD);
...
main (int ac, char **av)
{
	...
	if (verbose >= 0)
		...
		fprintf (stderr,
			"%s Population Generator (Version %d.%d.%d%s)\n%s\n",
			NAME, VERSION, RELEASE, MODIFICATION, PATCH, SECONDO_MOD);
		fprintf (stderr, "Copyright %s %s\n", TPC, C_DATES);
		}
