...
/* 
ROTATE[_]LEFT rotates x left n bits.
...
Encodes input (uint32[_]t) into output (unsigned char). Assumes len is
a multiple of 4.
...
Decodes input (unsigned char) into output (uint32[_]t). Assumes len is
a multiple of 4.
...
char * MD5::unix_encode(const char *pw, const char *salt)
{
	MD5_CTX	ctx,ctx1;
	unsigned long l;
	int sl, pl;
