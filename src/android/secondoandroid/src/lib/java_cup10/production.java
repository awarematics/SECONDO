 ...
 * 
 * @see     java_cup10.production_part
 * @see     java_cup10.symbol_part
 * @see     java_cup10.action_part
...
public class production {
  ...
  protected String make_declaration(
				    ...
				    int     offset)
    {
      ...
      if (emit.lr_values())
        ret = "\t\tint " + labelname + "left = ((java_cup10.runtime.Symbol)" + 
	  emit.pre("stack") + ".elementAt(" + emit.pre("top") + 
	  "-" + offset + ")).left;\n" +
	  "\t\tint " + labelname + "right = ((java_cup10.runtime.Symbol)" + 
	  emit.pre("stack") + ".elementAt(" + emit.pre("top") +
	  "-" + offset + ")).right;\n";
      else ret = "";
	...
	return ret + "\t\t" + stack_type + " " + labelname + " = (" + stack_type + 
	  ")((" + "java_cup10.runtime.Symbol) " + emit.pre("stack") + ".elementAt(" + emit.pre("top") 
	  + "-" + offset + ")).value;\n";
